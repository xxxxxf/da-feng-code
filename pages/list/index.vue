	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				// 解决 H5跨域问题
				// #ifdef H5
					url = `/HistoryToday/LookUp?page=1&type=2&rows=50&key=${key}&yue=${yue}&ri=${ri}`
				// #endif
				// #ifndef H5
			...
			getLang(flag) {
				...
				let url = ''
				// #ifdef H5
					url = `/MingRenMingYan/LookUp?keyword=${encodeURIComponent(this.searchVal?this.searchVal:char)}&key=${key}&page=${this.page}`
				// #endif
				// #ifndef H5
